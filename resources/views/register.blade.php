  ...
  <body class="min-h-screen flex flex-col justify-between items-center bg-[#78b3ce] text-[#040720] overflow-hidden">
    <main class="flex-1 flex justify-center items-center w-full">
        <div class="flex flex-col justify-center items-center bg-white p-8 rounded-2xl">
            ...
            <form action="{{ route("registercheck") }}" method="post" class="flex flex-col justify-center items-center">
                @csrf
                <div class="mb-2">
                ...
                <input type="submit" name="signup" value="sign up" class="px-5 py-2 bg-[#f96e2a] text-white rounded-xl hover:bg-[#f15e15] transition duration-200 w-[100px]"><br>
            </form>

            <div>
                <p class="pt-4">Already have an account? <a href="{{ route('login') }}" class="text-[#78b3ce]">Sign in</a></p>
            </div>
        </div>
    </main>
